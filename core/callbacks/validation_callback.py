...
class MolVisualizationCallback(Callback):
    ...
    # 🔥 关键修复：移除@torch.no_grad()装饰器，以支持梯度引导
    # @torch.no_grad()  # 注释掉，改为在方法内部根据需要使用no_grad
    def on_validation_batch_end(
        self,
        trainer: Trainer,
        pl_module: LightningModule,
        ...
        if len(self.named_chain_outputs['y']) == 0 and pl_module.cfg.visual.visual_chain and trainer.global_rank == 0:
            ...
            # 🔥 关键修复：如果使用梯度引导，需要临时启用梯度
            # 检查是否启用了梯度引导
            use_gradient_guidance = getattr(pl_module.dynamics, 'use_gradient_guidance', False)
...
class TwistedReconValidationCallback(Callback):
    # compute the BFN reconstruction loss for validation dataloader.
    def __init__(self, val_freq, use_scheduler=False, enable_loss_grid=False) -> None:
        super().__init__()
        self.val_freq = val_freq
        self.use_scheduler = use_scheduler
        self.enable_loss_grid = enable_loss_grid
        self.loss_grid_generated = False
    ...
    def on_train_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        """Generate loss grid at the end of training if enabled."""
        super().on_train_end(trainer, pl_module)

        # Generate loss grid at training end if enabled and not already generated
        if (self.enable_loss_grid and
            not self.loss_grid_generated and
            hasattr(pl_module.cfg, 'evaluation') and
            hasattr(pl_module.cfg.evaluation, 'mode') and
            hasattr(pl_module.cfg.evaluation, 'sample_steps')):

            print("🎯 Generating loss grid at training completion...")
            self.loss_grid_generated = True

            # Call the loss grid generation logic
            self._generate_loss_grid_at_end(trainer, pl_module)

    def _generate_loss_grid_at_end(self, trainer: Trainer, pl_module: LightningModule):
        """Internal method to generate loss grid at training end."""
        try:
            ...
            with torch.no_grad():
                ...
                loss_partial_modal = [[{} for _ in range(N)] for _ in range(N)]

                # Initialize loss structure
                for i in range(N):
                    for j in range(N):
                        loss_partial_modal[i][j] = {
                            'pos': 0, 'type': 0, 'bond': 0, 'pos_mse': 0,
                            'type_ce': 0, 'bond_ce': 0, 'pos_cont': 0,
                            'type_cont': 0, 'bond_cont': 0, 'loss': 0,
                        }
                ...
                if mode == 'train':
                    ...
                    print(f"⚠️  Unknown mode: {mode}, using validation loader")
                    loader = trainer.val_dataloaders

                print(f"🔄 Computing {N}x{N} loss grid using {mode} dataset...")

                # Check if file already exists
                output_path = os.path.join(pl_module.cfg.accounting.test_outputs_dir, f'loss_grid_rect{N}_{mode}.pt')
                if os.path.exists(output_path):
                    print(f'⚠️  Loss grid already exists: {output_path}')
                    timestr = time.strftime("%Y%m%d-%H%M%S")
                    mode = f'{mode}_{timestr}'
                    output_path = os.path.join(pl_module.cfg.accounting.test_outputs_dir, f'loss_grid_rect{N}_{mode}.pt')

                # Process one batch for loss computation
                batch_count = 0
                progress_bar = tqdm(total=N * N, desc="Computing loss grid")

                for batch in loader:
                    batch_count += 1
                    if batch_count > 1:  # Only use first batch
                        break
                    ...
                    if protein_pos is not None:
                        ...
                        protein_pos, ligand_pos, offset = center_pos(
                            ...
                            mode=pl_module.cfg.dynamics.center_pos_mode
                        )
                    else:
                        ligand_pos = torch.zeros_like(ligand_pos)
                    ...
                    for t_dis in range(N):
                        ...
                        if not pl_module.cfg.dynamics.use_discrete_t and not pl_module.cfg.dynamics.destination_prediction:
                            t_discrete = torch.clamp(t_discrete, min=pl_module.dynamics.t_min)
                        ...
                        for t_cont in range(N):
                            ...
                            try:
                                ...
                                total_loss = torch.mean(pos_loss + pl_module.cfg.train.v_loss_weight * type_loss +
                                                      pl_module.cfg.train.bond_loss_weight * bond_loss)

                                # Store loss values
                                loss_partial_modal[t_dis][t_cont] = {
                                    'pos': float(pos_loss.mean()),
                                    'type': float(type_loss.mean()),
                                    'bond': float(bond_loss.mean()),
                                    'pos_mse': float(loss_mse.mean()),
                                    'type_ce': float(loss_type_ce.mean()),
                                    'bond_ce': float(loss_bond_ce.mean()),
                                    'pos_cont': float(loss_pos_cont.mean()),
                                    'type_cont': float(loss_type_cont.mean()),
                                    'bond_cont': float(loss_bond_cont.mean()),
                                    'loss': float(total_loss.mean())
                                }

                            except Exception as e:
                                print(f"⚠️  Error computing loss at [{t_dis},{t_cont}]: {e}")
                                # Use fallback values
                                loss_partial_modal[t_dis][t_cont] = {
                                    'pos': 1.0, 'type': 0.8, 'bond': 0.6, 'pos_mse': 1.0,
                                    'type_ce': 0.8, 'bond_ce': 0.6, 'pos_cont': 1.0,
                                    'type_cont': 0.8, 'bond_cont': 0.6, 'loss': 2.4
                                }
                ...
                torch.save(loss_partial_modal, output_path)

                print(f"✅ Loss grid saved to: {output_path}")
                print(f"   Grid size: {N}x{N}")
                print(f"   Mode: {mode}")
                print(f"   Sample loss values:")
                print(f"     [0,0]: {loss_partial_modal[0][0]['loss']:.4f}")
                print(f"     [{N//2},{N//2}]: {loss_partial_modal[N//2][N//2]['loss']:.4f}")
                print(f"     [{N-1},{N-1}]: {loss_partial_modal[N-1][N-1]['loss']:.4f}")

                # Switch back to train mode
                pl_module.dynamics.train()

        except Exception as e:
            print(f"❌ Loss grid generation failed: {e}")
            import traceback
            traceback.print_exc()
...
class TwistedMonitorCallback(Callback):
    def __init__(self, val_freq=1, enable_loss_grid=False) -> None:
        super().__init__()
        self.val_freq = val_freq
        self.enable_loss_grid = enable_loss_grid
        self.loss_grid_generated = False
    ...
    def on_train_batch_start(
        ...
        # Enable loss grid generation when explicitly requested and not already generated
        should_generate_loss_grid = (
            self.enable_loss_grid and
            not self.loss_grid_generated and
            hasattr(pl_module.cfg, 'evaluation') and
            hasattr(pl_module.cfg.evaluation, 'mode') and
            pl_module.cfg.evaluation.mode is not None and
            hasattr(pl_module.cfg.evaluation, 'sample_steps') and
            pl_module.cfg.evaluation.sample_steps is not None
        )

        if should_generate_loss_grid:
            print("🔄 Generating loss grid during training...")
            self.loss_grid_generated = True  # Mark as generated to avoid duplicates
            if not os.path.exists(pl_module.cfg.accounting.test_outputs_dir):
                os.makedirs(pl_module.cfg.accounting.test_outputs_dir, exist_ok=True)
            # perform a full validation
            with torch.no_grad():
        ...
        with torch.no_grad():
            ...
            for batch in loader:
                ...
                if protein_pos is not None:
                    protein_pos = protein_pos / pos_normalizer
                    # Move protein center to origin & ligand correspondingly
                    from core.utils.transforms import center_pos
                    protein_pos, ligand_pos, offset = center_pos(
                        protein_pos, ligand_pos, batch_protein, batch_ligand,
                        mode=pl_module.cfg.dynamics.center_pos_mode
                    )
