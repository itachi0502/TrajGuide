...
# Import SOTA UFF error handling
try:
    from core.evaluation.utils.rdkit_uff_fix import safe_uff_optimize_molecule
    HAS_UFF_FIX = True
except ImportError:
    HAS_UFF_FIX = False
...
def get_rdkit_rmsd(mol, n_conf=20, random_seed=42):
    ...
    try:
        ...
        for confId in confIds:
            # Use SOTA UFF optimization with error handling
            if HAS_UFF_FIX:
                # Create a copy for this conformer
                conf_mol = Chem.Mol(mol3d)
                success, optimized_mol, method = safe_uff_optimize_molecule(conf_mol, verbose=False)
                if success and optimized_mol is not None:
                    # Copy optimized coordinates back to original molecule
                    try:
                        optimized_conf = optimized_mol.GetConformer(0)
                        original_conf = mol3d.GetConformer(confId)
                        for i in range(optimized_conf.GetNumAtoms()):
                            pos = optimized_conf.GetAtomPosition(i)
                            original_conf.SetAtomPosition(i, pos)
                    except Exception as copy_error:
                        print(f"⚠️  Failed to copy optimized coordinates: {copy_error}")
                        # Fallback to original UFF
                        try:
                            AllChem.UFFOptimizeMolecule(mol3d, confId=confId)
                        except Exception as uff_error:
                            print(f"⚠️  UFF fallback also failed: {uff_error}")
                else:
                    # Fallback to original UFF
                    try:
                        AllChem.UFFOptimizeMolecule(mol3d, confId=confId)
                    except Exception as uff_error:
                        print(f"⚠️  UFF optimization failed for confId {confId}: {uff_error}")
            else:
                # Original method with error handling
                try:
                    AllChem.UFFOptimizeMolecule(mol3d, confId=confId)
                except Exception as uff_error:
                    print(f"⚠️  UFF optimization failed for confId {confId}: {uff_error}")
            ...
            try:
                ...
                print(f"⚠️  RMSD calculation failed for confId {confId}: {rmsd_error}")
                rmsd_list.append(np.nan)

        # mol3d = Chem.RemoveHs(mol3d)
        rmsd_list = np.array(rmsd_list)
        # Filter out NaN values
        valid_rmsd = rmsd_list[~np.isnan(rmsd_list)]
        if len(valid_rmsd) > 0:
            return [np.max(valid_rmsd), np.min(valid_rmsd), np.median(valid_rmsd)]
        else:
            return [np.nan, np.nan, np.nan]
    except Exception as e:
        print(f"⚠️  get_rdkit_rmsd failed: {e}")
        return [np.nan, np.nan, np.nan]
