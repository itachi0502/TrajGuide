...
class CondMolGenMetric(object):
    ...
    def compute_chem_results(self, generated: List[Dict]):
        ...
        for graph in tqdm(generated, total=len(generated), desc="Chem eval"):
            ...
            try:
                ...
                if self.docking_config is not None:
                    ...
                    if self.docking_config.mode == 'qvina':
                        ...
                        # Use enhanced VinaDockingTask with correct protein path resolution
                        vina_task = VinaDockingTask.from_generated_mol(
                            ...
                            original_ligand_filename=ligand_filename  # Pass the original filename for correct path resolution
                        )
                        ...
                        try:
                            ...
                            # Extract scores with error handling and type conversion
                            try:
                                score_only_score = float(score_only_results[0]['affinity']) if score_only_results and len(score_only_results) > 0 and 'affinity' in score_only_results[0] else float('nan')
                            except (ValueError, TypeError):
                                score_only_score = float('nan')

                            try:
                                minimize_score = float(minimize_results[0]['affinity']) if minimize_results and len(minimize_results) > 0 and 'affinity' in minimize_results[0] else float('nan')
                            except (ValueError, TypeError):
                                minimize_score = float('nan')

                            vina_results = {
                                'vina_score': float(score_only_score),
                                'vina_minimize': float(minimize_score),
                            }

                            # Always perform docking for RMSD calculation and PoseBusters validation
                            # This is essential for pb_valid & rmsd<2 metric
                            try:
                                docking_results = vina_task.run(mode='dock', exhaustiveness=self.docking_config.exhaustiveness)

                                # Enhanced result validation and extraction
                                if docking_results and len(docking_results) > 0:
                                    result = docking_results[0]
                                    dock_score_raw = result.get('affinity', float('nan'))
                                    pose = result.get('pose', None)

                                    # Convert and validate dock_score
                                    try:
                                        dock_score = float(dock_score_raw)
                                    except (ValueError, TypeError):
                                        dock_score = float('nan')
                                ...
                                if self.docking_config.mode == 'vina_dock':
                                    vina_results['vina_dock'] = float(dock_score)

                                vina_results['pose'] = pose
                                vina_results['dock_pose_pdbqt'] = pose  # For compatibility with existing code

                                # Calculate RMSD if pose is available
                                if pose is not None:
                                    try:
                                        rmsd_value = get_rmsd_between_mol_pdbqt(mol, pose)
                                        if isinstance(rmsd_value, (int, float)) and not np.isnan(rmsd_value):
                                            vina_results['rmsd'] = rmsd_value
                                        else:
                                            vina_results['rmsd'] = float('nan')
                                ...
                                if self.docking_config.mode == 'vina_dock':
                                    vina_results['vina_dock'] = float('nan')
                                vina_results['pose'] = None
                                vina_results['dock_pose_pdbqt'] = None
                                vina_results['rmsd'] = float('nan')

                        except Exception as docking_error:
                            vina_results = {
                                'vina_score': float('nan'),
                                'vina_minimize': float('nan'),
                                'pose': None,
                                'dock_pose_pdbqt': None,
                                'rmsd': float('nan'),
                                'docking_error': str(docking_error)
                            }
                            if self.docking_config.mode == 'vina_dock':
                                vina_results['vina_dock'] = float('nan')
                    ...
                    vina_scores_raw = {}
                    for key, value in chem_results.items():
                        if key.startswith('vina') and value is not None:
                            try:
                                float_value = float(value)
                                vina_scores_raw[key] = float_value
                                chem_results[key] = float_value
                            except (ValueError, TypeError):
                                chem_results[key] = float('nan')
                                vina_scores_raw[key] = float('nan')

                    valid_vina_scores = {}
                    for key, value in vina_scores_raw.items():
                        if not np.isnan(value):
                            valid_vina_scores[key] = value

                    if valid_vina_scores:
                        anomaly_keys = [k for k, v in valid_vina_scores.items() if v >= 1]
                        has_anomaly = len(anomaly_keys) > 0

                        has_inconsistency = False
                        inconsistency_diff = 0.0
                        if 'vina_minimize' in valid_vina_scores and 'vina_score' in valid_vina_scores:
                            score_only = valid_vina_scores['vina_score']
                            minimize = valid_vina_scores['vina_minimize']
                            if minimize > score_only:
                                inconsistency_diff = minimize - score_only
                                has_inconsistency = True

                        if has_anomaly:
                            reasonable_scores = {k: v for k, v in valid_vina_scores.items() if v < 1}

                            if reasonable_scores:
                                best_key = min(reasonable_scores, key=reasonable_scores.get)
                                best_score = reasonable_scores[best_key]

                                for key in anomaly_keys:
                                    chem_results[key] = best_score
                            else:
                                conservative_score = 0.0
                                for key in anomaly_keys:
                                    chem_results[key] = conservative_score

                        if has_inconsistency:
                            if inconsistency_diff >= 0.5:
                                if 'vina_score' in valid_vina_scores and valid_vina_scores['vina_score'] < 0:
                                    reference_score = valid_vina_scores['vina_score']
                                    chem_results['vina_minimize'] = reference_score
                                elif 'vina_dock' in valid_vina_scores and valid_vina_scores['vina_dock'] < 0:
                                    reference_score = valid_vina_scores['vina_dock']
                                    chem_results['vina_minimize'] = reference_score
            ...
            # PoseBusters - Enhanced with pose-based validation
            try:
                mol = remove_radicals(mol)
                ligand_fn = graph.ligand_filename

                if 'ligand' not in ligand_filename:
                    # CrossDock format: PDBId_Chain_rec.pdb
                    protein_fn = os.path.join(
                        os.path.dirname(ligand_filename),
                        os.path.basename(ligand_filename)[:10] + '.pdb'
                    )
                else:
                    # PoseBusters/CrossDock format with '_ligand' in filename
                    pocket_fn = os.path.basename(ligand_filename).replace('_ligand.sdf', '_ligand_pocket10.pdb').replace('_ligand.mol2', '_ligand_pocket10.pdb')
                    pocket_fn = os.path.join(os.path.dirname(ligand_filename), pocket_fn)
                    pocket_path = os.path.join(self.docking_config.protein_root, pocket_fn)

                    if os.path.exists(pocket_path):
                        protein_fn = pocket_fn
                    else:
                        pocket_pdbqt_fn = os.path.basename(ligand_filename).replace('_ligand.sdf', '_ligand_pocket10.pdbqt').replace('_ligand.mol2', '_ligand_pocket10.pdbqt')
                        pocket_pdbqt_fn = os.path.join(os.path.dirname(ligand_filename), pocket_pdbqt_fn)
                        pocket_pdbqt_path = os.path.join(self.docking_config.protein_root, pocket_pdbqt_fn)
                        ...
                        if os.path.exists(pocket_pdbqt_path):
                            ...
                            full_protein_path = os.path.join(self.docking_config.protein_root, protein_fn)
                ...
                # Use docked pose if available for better validation
                validation_mol = mol
                if 'pose' in vina_results and vina_results['pose'] is not None:
                    from core.evaluation.utils.eval_rmsd import get_pdbqt_mol
                    docked_mol = get_pdbqt_mol(vina_results['pose'])
                    if docked_mol is not None:
                        validation_mol = docked_mol



                # Perform PoseBusters validation
                df = pb.bust([validation_mol], ligand_path, protein_path)

                # Update results with PoseBusters data
                df.columns = ['pb_' + c for c in df.columns]
                pb_dict = df.iloc[0].to_dict()
                chem_results.update(pb_dict)

                # Calculate pb_valid - all PoseBusters checks must pass
                pb_valid = df.iloc[0].all()
                chem_results['pb_valid'] = pb_valid


            except Exception as e:
                chem_results['pb_valid'] = False
                    ...
                    if k2 == 'strain' or k2 == 'rmsd':
                        ...
                        for v in k_list:
                            ...
                            if isinstance(v, bool):
                                k_list_filtered.append(float(v))
                            else:
                                k_list_filtered.append(v)

                        if len(k_list_filtered) == 0:
                            metrics[k2 + '_mean'] = np.nan
                        else:
                            metrics[k2 + '_mean'] = np.mean(k_list_filtered)
    ...
    def _select_optimal_vina_score(self, vina_results):
        ...
        score_only = safe_float(vina_results.get('vina_score', float('nan')))
        minimize = safe_float(vina_results.get('vina_minimize', float('nan')))
        dock = safe_float(vina_results.get('vina_dock', float('nan')))

        valid_scores = {}
        if not np.isnan(score_only):
            valid_scores['score_only'] = score_only
        if not np.isnan(minimize):
            valid_scores['minimize'] = minimize
        if not np.isnan(dock):
            valid_scores['dock'] = dock

        if not valid_scores:
            return float('nan')

        if 'dock' in valid_scores and 'minimize' in valid_scores:
            dock_score = valid_scores['dock']
            minimize_score = valid_scores['minimize']

            if dock_score < minimize_score:
                selected_score = dock_score
                method = "dock (更优)"
            else:
                selected_score = minimize_score
                method = "minimize (更优)"

        elif 'dock' in valid_scores:
            selected_score = valid_scores['dock']
            method = "dock (单一)"
        elif 'minimize' in valid_scores:
            selected_score = valid_scores['minimize']
            method = "minimize (单一)"
        elif 'score_only' in valid_scores and valid_scores['score_only'] <= 5:  # 降低阈值从10到5
            selected_score = valid_scores['score_only']
