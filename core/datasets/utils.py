...
class PDBProteinNew(object):
    ...
    AA_NAME_NUMBER.update({
        'PTR': 19,  # Phosphotyrosine (modified TYR)
        'SEP': 18,  # Phosphoserine (modified SER)
        'TPO': 17,  # Phosphothreonine (modified THR)
        'HYP': 15,  # Hydroxyproline (modified PRO)
        'MSE': 11,  # Selenomethionine (modified MET)
    })
...
def parse_sdf_file(path, add_hydrogen=False, kekulize=False):
    ...
    # Use NumPy 1.20+ compatible integer type
    try:
        element = np.array(element, dtype=np.int64)
        charge = np.array(charge, dtype=np.int64)
