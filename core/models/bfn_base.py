...
class BFNBase(nn.Module):
    ...
    def continuous_var_bayesian_update(self, t, sigma1, x):
        ...
        if x.dim() != 2:
            raise ValueError(f"continuous_var_bayesian_update: x维度错误 {x.shape}，期望2维 [N, D]")
        ...
        if t.dim() == 2 and t.size(1) == 1:
            ...
            raise ValueError(f"continuous_var_bayesian_update: t维度错误 {t.shape}，期望 [N, 1] 或 [N]")

        # Eq.(77): p_F(θ|x;t) ~ N (μ | γ(t)x, γ(t)(1 − γ(t))I)
        # sigma1.requires_grad = True
        # x.requires_grad = True
        gamma = 1 - torch.pow(sigma1, 2 * t)  # [N]


        gamma_expanded = gamma.unsqueeze(-1)  # [N, 1]

        mu = gamma_expanded * x + torch.randn_like(x) * torch.sqrt(gamma_expanded * (1 - gamma_expanded))
    ...
    def discrete_var_bayesian_update(self, t, beta1, x, K, eps=None):
        ...
        """
        if x.dim() != 2:
            raise ValueError(f"discrete_var_bayesian_update: x维度错误 {x.shape}，期望2维 [N, K]")
        ...
        if t.dim() == 2 and t.size(1) == 1:
            ...
            raise ValueError(f"discrete_var_bayesian_update: t维度错误 {t.shape}，期望 [N, 1] 或 [N]")
        ...
        beta_expanded = beta.unsqueeze(-1)  # [N, 1]

        mean = beta_expanded * (K * one_hot_x - 1)  # [N, 1] * [N, K] -> [N, K]
        std = (beta_expanded * K).sqrt()  # [N, 1]
        if eps is None:
            eps = torch.randn_like(mean)
        y = mean + std * eps
        theta = F.softmax(y, dim=-1)

        if theta.dim() != 2 or theta.shape != x.shape:
            raise ValueError(f"discrete_var_bayesian_update: 输出维度异常 {theta.shape}，期望 {x.shape}")
